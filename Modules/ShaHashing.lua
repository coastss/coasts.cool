...
do
    local function Mul(src1, src2, factor, result_length)
        ...
        for j = 1, result_length do
            ...
            result[j] = digit
            carry = math.floor(carry / 16777216)
    ...
    repeat
        ...
        repeat
            ...
            if d * d > p then
                idx = idx + 1
                local root = p^(1/3)
                local R = Mul({math.floor(root * 2^40)}, one, 1, 2)
                local _, delta = Mul(R, Mul(R, R, 1, 4), -1, 4)
                local hi = R[2] % 65536 * 65536 + math.floor(R[1] / 256)
                local lo = R[1] % 256 * 16777216 + math.floor(delta * (2^-56 / 3) * root / p)
                sha2_K_hi[idx], sha2_K_lo[idx] = hi, lo
                if idx < 17 then
                    root = p^(1/2)
                    R = Mul({math.floor(root * 2^40)}, one, 1, 2)
                    _, delta = Mul(R, R, -1, 2)
                    hi = R[2] % 65536 * 65536 + math.floor(R[1] / 256)
                    lo = R[1] % 256 * 16777216 + math.floor(delta * 2^-17 / root)
        ...
        if text_part then
            if tail then
                ...
                if tail ~= "" and #tail + #text_part >= 64 then
                    offs = 64 - #tail
                    Sha256_Feed_64(H, sha2_K_hi, tail..string.sub(text_part, 1, offs), common_W, 0, 64)
                    tail = ""
                end
                local size = #text_part - offs
                local size_tail = size % 64
                Sha256_Feed_64(H, sha2_K_hi, text_part, common_W, offs, size - size_tail)
                tail = tail..string.sub(text_part, #text_part + 1 - size_tail)
                return Partial
            else
        ...
        if text_part then
            if tail then
                ...
                if tail ~= "" and #tail + #text_part >= 128 then
                    offs = 128 - #tail
                    Sha512_Feed_128(H_lo, H_hi, sha2_K_lo, sha2_K_hi, tail..string.sub(text_part, 1, offs), common_W, 0, 128)
                    tail = ""
                end
                local size = #text_part - offs
...
local function SecureBeach(str)
    return Base64.Encode(Sha512ext(512, str))
end

return {
    Sha = Sha512ext,
    SecureBeach = SecureBeach,
    GamerSha = GamerSha
}