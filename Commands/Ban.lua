...
local UserToBan = Utilities.ParseDiscordMention(table.remove(Arguments, 1))
local BanReason = table.concat(Arguments, " ")

if #BanReason < 5 then
    return Message:reply(("%s, please specify a reason."):format(Utilities.Constants.MentionUser:format(AuthorId)))
else
    local Success, Error = Message.member.guild:banUser(UserToBan, BanReason)

    if Success then
        return Message:reply(Utilities.CreateEmbed("Coasting", {
            "Successfully banned user:",
            Utilities.Constants.MentionUser:format(UserToBan),
            "Reason:",
            BanReason
